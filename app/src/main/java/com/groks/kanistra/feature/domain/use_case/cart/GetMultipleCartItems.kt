...
class GetMultipleCartItems @Inject constructor(
    private val repository: KanistraRepository
) {
    operator fun invoke(ids: String): Flow<Resource<List<CartItem>>> = flow {
        try {
            emit(Resource.Loading())
            val cartItems = repository.getMultipleCartItems(ids)
